...
We need to enlarge the image to see it. Fiji displays the eight you see when you open the file 
as closely as possible to the original data. But 15 pixels on the screen is 
...
You'll need to use the magnifying glass again.)
...
(i.e. how much memory is allocated to store a single pixel's data - see below -- [Bits & Bytes](#bits-and-bytes)) 
...
and because the format is so widely used,
...
![Histograms of the two enlarged images](fig/image-basics/quality-histogram.jpg){alt='histograms of the above images'}
...
Fiji tries to read metadata from files that you open and includes 
metadata when saving new images but may fail to keep
certain metadata fields, depending on the file format you choose 
when saving.
...
Fiji provides a way to display or explore the metadata
...
e.g., [ImageMagick](https://imagemagick.org/index.php).
You may want to explore these options if you need to work with
the metadata of your images.  
In Fiji, the metadata of an image can be accessed via the menu, using: 
...
an open-source product released by the [Open Microscopy Environment](https://wwww.openmicroscopy.org), 
is bundled with Fiji, making it easy to open proprietary files by overriding Fiji's 
...
- Most frequently, colour digital images use an additive RGB model, with eight bits for the red, green, and blue channels.
- Images are numeric arrays, although the array may be hidden from the end user.
- In Fiji, values for each of the channels are specified as an (r,g,b) triplet.
- Lossless compression retains all the details in an image. 
- Lossy compression results in loss of some of the original image detail.
...
- Depending on the camera or sensor, various useful pieces of information (image metadata) may be stored in the image file.
